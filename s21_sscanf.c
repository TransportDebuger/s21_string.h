...
static void set_width(const char **format, int *width) {
...
static void set_length(const char **format, int *length) {
  *length = 0;
  switch (**format) {
    case 'h':
      *length = 1;
...
void select_specifiers(va_list args, const char **str, int *read_values,
...
int s21_main_function(const char *str, const char *format, va_list *ap) {
  ...
  while (*format != '\0' && *str != '\0') {
    ...
    if (*format == '%') {
      ...
      select_specifiers(*ap, &str, &read_values, &width, length, &format);
...
int s21_sscanf(const char *str, const char *format, ...) {
  ...
  va_list ap;
  va_start(ap, format);
  int rez = s21_main_function(str, format, &ap);
  va_end(ap);
  return rez;
}
